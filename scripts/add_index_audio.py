...
def add_player(match):
    ...
    player = (
        f'<div class="module-audio"><span class="audio-label">播放摘要</span>'
        f'<audio controls preload="none" aria-label="播放本模組摘要" src="audio_summaries/{filename}"></audio></div>'
    )
    card = re.sub(r'<a class="map-card" href="([^"]+)">', r'<div class="map-card"><a class="map-card-link" href="\1">', card, count=1)
    return card.replace("</a>", f"</a>{player}</div>", 1)
...
.map-card-link{display:block;color:var(--ink);text-decoration:none}
.map-card-link:hover,.map-card-link:focus-visible{color:var(--ink)}
.module-audio{display:flex;align-items:center;gap:10px;margin-top:18px;padding-top:14px;border-top:1px solid var(--line)}
.audio-label{color:var(--muted);font-size:12px;white-space:nowrap}
...
updated = re.sub(r'<script>\s*document\.querySelectorAll\(\'.module-audio audio\'\).*?</script>', '', updated, count=1, flags=re.S)
