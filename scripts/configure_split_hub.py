...
def configure() -> None:
    ...
    public_start = config.get("public_module_start", start)
    ...
    for offset, card in enumerate(selected):
        selected[offset] = re.sub(r"MODULE \d{2}", f"MODULE {public_start + offset:02d}", card, count=1)
    index = re.sub(
    ...
    admin = re.sub(
        ...
        lambda m: (m.group(0).replace(f"Module {int(m.group(1)):02d}", f"Module {public_start + int(m.group(1)) - start:02d}")
                   if start <= int(m.group(1)) <= end else ""),
        admin,
    )
