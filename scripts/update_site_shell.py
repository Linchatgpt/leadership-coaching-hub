...
LEGACY_BRAND_NAMES = ("卓越領導者教練學習中心", "卓越領導力學習中心", "卓越領導力©核心課程")
BRAND_NAME = "卓越教練力©核心課程"
BRAND_MARK = "XC"
...
def update_page(source: str, *, home: bool = False) -> str:
    for old_name in LEGACY_BRAND_NAMES:
        source = source.replace(f'<i>LPI</i> {old_name}', f'<i>{BRAND_MARK}</i> {BRAND_NAME}')
        source = source.replace(f'<i>XL</i> {old_name}', f'<i>{BRAND_MARK}</i> {BRAND_NAME}')
        source = source.replace(f'<i>XC</i> {old_name}', f'<i>{BRAND_MARK}</i> {BRAND_NAME}')
