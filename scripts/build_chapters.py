...
brand_name=cfg.get('project_title','卓越教練力©核心課程')
brand_mark=cfg.get('site_shell',{}).get('brand_mark','XC')
module_start,module_end=cfg.get('module_range',[1,11])
public_module_start=cfg.get('public_module_start',module_start)
chapters=[
...
seo=conf.get('seo',{})
shell=(ROOT/'templates/chapter_template.html').read_text().replace('{{PAGE_TITLE}}',title).replace('{{META_DESCRIPTION}}',seo.get('description',title+'｜卓越教練力©核心課程')).replace('{{META_KEYWORDS}}',','.join(seo.get('keywords',[]))).replace('{{OG_TITLE}}',seo.get('title',title+'｜卓越教練力©核心課程')).replace('{{CSS}}',(ROOT/'assets/chapter.css').read_text()+(ROOT/'assets/editor-runtime.css').read_text()).replace('{{BODY}}',body).replace('{{CONFIG}}',json.dumps(conf,ensure_ascii=False)).replace('{{RUNTIME}}',(ROOT/'assets/editor-runtime.js').read_text()+'\n'+runtime)
...
cards=''.join(f'<a class="map-card" href="{prefix}_Chapter{i:02d}.html"><small>MODULE {public_module_start + i - module_start:02d}</small><h3>{c[0]}</h3><p>{c[2]}</p><span>開始本模組 →</span></a>' for i,c in selected_modules)
custom='''<div class="top"><a class="brand" href="index.html" aria-label="學習中心首頁"><i>XC</i> 卓越教練力©核心課程</a></div><main class="home"><section class="home-hero"><small>FROM FEEDBACK TO PRACTICE</small><h1>把領導力，<em>練成日常。</em></h1><p>一個以領導力教練與組織學習為核心的繁體中文個人學習中心。從五項領導實踐出發，走過教練能力與流程，最後回到你自己的工作現場。</p><div class="journey"><b>11 章</b><span>從領導挑戰到教練精熟</span><b>1 個循環</b><span>閱讀 → 觀察 → 實驗 → 反思</span></div></section><section><small>LEARNING MAP</small><h2>選一章，帶一個真實議題進來。</h2><div class="map">'''+cards+'''</div></section><section class="home-note"><h2>使用前請知道</h2><p>頁面中的自評題目、案例與工作工具都是發展用途，不是標準化量表或人資決策工具。你的紀錄只保存在目前瀏覽器／裝置中；請將人物與事件去識別化。</p></section></main><footer>卓越教練力©核心課程 · 個人學習原型</footer>'''
