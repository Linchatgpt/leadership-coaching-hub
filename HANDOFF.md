...
## Public module renumbering · 2026-08-04

The coaching hub now presents the former Modules 07–11 as Modules 01–05. Canonical filenames (`LPI_Coach_Chapter07.html` through `11`), internal chapter numbers, source paths, and localStorage keys remain unchanged for compatibility. `config.json.public_module_start` and the split builder preserve this mapping.

## Module 01 content · 2026-08-04

The former Chapter 07 / public Module 01 now has source-grounded content. Local extraction covered the source PDF Chapter 7 range beginning around PDF page 207. Research notes are in `reference_materials/chapters/chapter_07/01_core_concepts.md`; the canonical learner draft is `02_deep_reading_draft.md`. The HTML was rebuilt from Markdown and measures 5,688 rendered Chinese characters. The learner page uses original paraphrase, an original Taiwan workplace case, three tools, three reflection pauses, and a summary; it does not reproduce source definitions, tables, questions, or cases.

## Brand update · 2026-08-04

The site shell now uses `XC` and `卓越教練力©核心課程`, configured in `config.json`. `PART I`／`PART II` labels were removed from the public index cards and chapter hero labels. Canonical filenames and storage keys remain unchanged.

## Module 01 contract migration · 2026-08-04

Module 01 now follows the latest `build-source-book-learning-hub` contract. Added the hero start prompt, four-point `reading-brief`, collapsed three-question `quick-scan`, chapter-specific learning config, three source-grounded callouts, and the direct `.head + .simple-record.action-commitment` ending. Stable storage key `LPI_CoachChapter7` is unchanged.

## Module 02 content · 2026-08-04

The former Chapter 08 / public Module 02 now follows the same latest chapter contract. Source extraction covered PDF Chapter 8 beginning around page 233. Research notes are in `reference_materials/chapters/chapter_08/01_core_concepts.md`; the canonical draft is `02_deep_reading_draft.md`. The page includes 5,139 rendered Chinese characters, three callouts, the start prompt, four-point orientation, three-question quick scan, localized case, assessment, and direct action-commitment structure. Stable storage key `LPI_CoachChapter8` is unchanged.

Module 01 and Module 02 cases and assessments were subsequently redesigned from the corresponding source themes. Module 01 now uses an internal-coach precision-equipment scenario and four behavioral dimensions; Module 02 uses a fintech payment-risk scenario and five coaching-competency dimensions.

## Project split · 2026-08-04

The project root is `/Users/wes_mini/Projects/Leadership Coaching Hub` and exposes only Module 07–11 with its own index. Module 01–06 remain in `/Users/wes_mini/Projects/Leadership Principle hub`. Learner-facing labels use `MODULE`/`Module`; canonical filenames, internal `chapter_*` paths, and localStorage keys remain unchanged for compatibility.
...
## 2026-08-04｜模組03 HBR review 修訂

模組03（canonical Chapter09）已依 HBR review 落地：案例現在不是停在情境與提問，而是呈現一輪可推演的教練工作，包括三方協議、觀察／解讀／假設、兩週實驗、混合結果及下一輪調整。六個 C 的首次介紹加入工具定位與管理邊界說明。發展自評改為 15 題，五構面各 3 題，`assessment.version` 為 `2.1.0`。來源仍由 `reference_materials/chapters/chapter_09/02_deep_reading_draft.md` 與 `content/chapter_09_learning.json` 管理，HTML 已重新生成。Chrome 390px 實測通過，測試資料已不保存於頁面。
## 2026-08-04｜模組04待確認

模組04目前停在「深入閱讀稿完成、HBR review 已提出並落實修訂」階段，尚未生成網頁。來源核心筆記與 canonical 稿件分別位於 `reference_materials/chapters/chapter_10/01_core_concepts.md`、`02_deep_reading_draft.md`。文章主線是：辨識教練計畫失去節奏的原因，將行動嵌入自然工作日，以證據修復承諾與跟進。SEO metadata 已附於稿件正文後方。下一步是使用者確認內容後，建立模組04 learning JSON、callouts、案例與自評，再生成 Chapter10 HTML 並完成 Chrome 實測。
## 2026-08-04｜模組04網頁已生成

模組04（canonical Chapter10）已完成網頁設計。案例使用台北 18 間門市的連鎖醫療服務公司與區域營運主管怡安，聚焦排班壓力、服務風險、授權與教練行動修復，未沿用前面模組案例。內容來源為 `reference_materials/chapters/chapter_10/02_deep_reading_draft.md`，互動來源為 `content/chapter_10_learning.json` 與 `content/chapter_10_callouts.json`。頁面加入 SEO metadata；Chrome 390px 實測案例、15 題自評結果、文章長度與無水平溢出均通過。
## 2026-08-04｜SEO 與內部編輯資料邊界修正

模組04 已修正公開內容邊界：深入閱讀 renderer 遇到 `SEO 與網頁製作資料` 或 `附錄｜原始寫作指令` 即停止輸出；兩者只留在 canonical Markdown 供內部建置與交接。SEO description／keywords／Open Graph 仍保留於 HTML head。重新生成後文章公開文字約 5,029 字，Chrome 已確認兩段不出現在頁面可見文字中。
## 2026-08-04｜模組05已完成

模組05（canonical Chapter11）已完成。來源文件為 `reference_materials/chapters/chapter_11/01_core_concepts.md`，文章來源為 `02_deep_reading_draft.md`。學習者頁面以新竹醫療科技公司組織發展經理若晴為案例，建立內容／流程／個人發展三線模型與一季精熟計畫。互動設定在 `content/chapter_11_learning.json`，前置專欄在 `content/chapter_11_callouts.json`。Chrome 390px 實測通過，公開正文約 5,033 字；SEO 與原始寫作指令未出現在可見內容。
## 2026-08-04｜模組05 HBR review 修訂完成

模組05已依 HBR review 修訂：統一 SEO／頁面主標題、前置全文核心主張、合併口號式短結尾，並將案例提問改為學習策略取捨題。canonical 深入閱讀稿與 `content/chapter_11_learning.json` 已同步，HTML 重新生成；Chrome 390px 實測通過，公開正文約 5,087 字。
## 2026-08-04｜模組05入口

首頁已存在 MODULE 05 卡片並連結至 `LPI_Coach_Chapter11.html`。Chrome 實際點擊驗證成功，頁面顯示新版「教練如何持續提升專業能力？」標題。
## 2026-08-04｜工具欄位技能同步
...
首頁模組卡片左側現在只顯示文字「播放摘要」，不再顯示自製三角形；右側原生控制列保留完整播放功能。產生器與首頁已同步，公開部署完成。
## 2026-08-06｜單一音檔播放

首頁五個摘要播放器已實作同頁單一播放規則。原生控制列直接操作也會觸發全域 `play` 監聽，因此第二個音檔開始時，前一個會自動 pause。公開網站已驗證。
